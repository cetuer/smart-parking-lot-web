...
import { getUser, updateUserRole } from '@/api/system/user';
...
export default {
    ...
    if (userId) {
      ...
      getUser(userId).then(res => {
        ...
        listRole().then(response => {
          ...
          listRole(userId).then(res => {
            const tmpIds = res;
            this.$nextTick(() => {
              this.roles.forEach(row => {
                if (tmpIds.includes(row.id)) {
  ...
  methods: {
    ...
    submitForm() {
      ...
      updateUserRole({ userId: userId, roleIds: roleIds }).then(response => {
